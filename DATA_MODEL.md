...
### landingPage
Universal editorial/SEO landing built from an ordered `pageSections[]` block array (see the section catalog below). `pageType` (home / city / cityIndex / district / propertyType / investment / custom) determines routing and which linked-entity reference applies. Key fields: `enabled`, `title`, `slug` (required except home), `pageSections[]`, card overrides (`cardTitle`, `cardDescription`, `cardImage`) for landing carousels, `linkedCity` / `linkedDistrict` / `linkedPropertyType` (validated per pageType; one enabled city landing per city), `contentUpdatedAt` (freshness badge), `seo` (all 5 locales required for enabled pages).
