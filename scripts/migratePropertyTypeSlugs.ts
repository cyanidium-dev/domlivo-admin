/**
 * Migrate propertyType documents: add slug where missing.
 * Generates slug from title.en (or first available locale) using slugify pattern.
 *
 * Run: npx tsx scripts/migratePropertyTypeSlugs.ts
...
async function main() {
  ...
  >(`*[_type == "propertyType"]{ _id, title, slug }`)
  ...
  for (const doc of docs) {
    ...
    if (!source) {
      console.warn(`  Skip ${doc._id}: no title to derive slug`)
      continue
    }
  ...
  if (count > 0) {
    await tx.commit()
    console.log(`\nMigrated ${count} propertyType documents.`)
  } else {
    console.log('No propertyType documents needed migration.')
  }
}
